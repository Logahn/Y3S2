...
def display_holidays(holidays):
    if holidays:
        print('Праздники для выбранной страны:')
        for holiday in holidays:
            print(f"{holiday['date']} - {holiday['name']}")
    else:
        print('Не удалось восстановить праздничные дни.')
...
def prompt_country(supported_countries, last_chosen_country):
    ...
    print('Поддерживаемые страны:')
    for i, country in enumerate(supported_countries):
        print(f'{i+1}. {country}')
    while True:
        choice = input(
            'Введите номер страны, в которой вы хотите получить отпускные для: ')
        try:
            choice = int(choice)
