...
namespace CondorcetMethod
{
    public partial class Form1 : Form
    {
        ...
        private void btnAddResult_Click(object sender, EventArgs e)
        {
            ...
            {
                // Use the pairwise results to calculate the Condorcet winner
                string condorcetWinner = CalculateCondorcetWinner();

                // Display the Condorcet winner in a message box
                MessageBox.Show($"The Condorcet winner is {condorcetWinner}");
            }
        }
        ...
        private string CalculateCondorcetWinner()
        {
            // Check if any candidate beats all other candidates in pairwise comparisons
            for (int i = 0; i < pairwiseResults.GetLength(0); i++)
            {
                bool candidateWinsAllComparisons = true;
