...
namespace LR_2
{
	public partial class Form_Electr : Form
	{
		private string Cel;
		public Form_Electr(string Cel)
		{
			InitializeComponent();
			this.Cel = Cel;
		}

		private void Form_Electr_Load(object sender, EventArgs e)
		{

		}
		public string metod = "Электра";
		public int kriter;
		public int alt;
		float[] arr_krit = new float[15];
		float[] arr_alt = new float[15];
		int f = 0;
		private void button1_Click(object sender, EventArgs e)//Ввод критериев и альт
		{
		...
		private void button2_Click(object sender, EventArgs e)//Заполнение табл 1 и запись в массив
		{
			for (int i = 0; i < kriter; i++)
			{
		...
		private void button4_Click(object sender, EventArgs e) // кнопка следующего критерия
		{
			for (int i = 0; i < alt; i++)
			{
		...
		private void button3_Click(object sender, EventArgs e)// кнопка ОК для альтернатив
		{
			for (int i = 0; i < alt; i++)
			{
				arr_alt[i] += Convert.ToInt32(dataGridView2.Rows[i].Cells[1].Value) / arr_krit[f];
		...
		{
			// Добавить текстовое поле с результатом
