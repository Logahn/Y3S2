...
namespace LR_2
{
	public partial class Form_MAI : Form
	{
		private string Cel;
		public Form_MAI(string Cel)
		{
			InitializeComponent();
			this.Cel = Cel;
		}
		...
		private void button2_Click_1(object sender, EventArgs e)
		{
			for (int i = 0; i < kriter; i++)
			{
				arr_krit[i] = (float)Math.Exp(0.7 * Convert.ToInt32(dataGridView1.Rows[i].Cells[1].Value));
			}
		...
		private void button4_Click_1(object sender, EventArgs e)
		{
			for (int i = 0; i < alt; i++)
			{
				arr_alt[i] += (float)Math.Exp(0.7 * (Convert.ToInt32(dataGridView2.Rows[i].Cells[1].Value) / arr_krit[f]));
